...
pub fn shaderInvocation(
    ...
    defer freeOwnedInputs(allocator, fragment_inputs);

    const derivatives = derivative_inputs;
    defer if (derivatives) |owned_derivatives| {
    ...
    const pipeline = draw_call.renderer.state.pipeline orelse return SpvRuntimeError.InvalidSpirV;
